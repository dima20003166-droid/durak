...
const ProfileModal = ({ user, loading, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-bg/70 z-50 flex items-center justify-center" onClick={onClose}>
      <div
        className="bg-surface p-6 rounded-xl border border-border w-full max-w-md text-text"
        onClick={e => e.stopPropagation()}
      >
        {loading ? (
          <div className="flex flex-col items-center">
            <Lottie loop animationData={loadingAnim} className="w-32 h-32" />
            <div className="mt-2 text-muted">Загрузка...</div>
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-3 gap-3 mt-4 text-center">
              <div className="bg-surface rounded p-2">
                ...
                <div className="text-lg font-bold">{stats.games || 0}</div>
              </div>
              <div className="bg-surface rounded p-2">
            ...
            <div className="mt-6 text-right">
              <button onClick={onClose} className="px-4 py-2 bg-primary hover:bg-primary/80 rounded text-text">
                Закрыть
              </button>
            </div>
          </>
