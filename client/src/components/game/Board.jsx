...
/**
 * Центральная зона стола: колода/козырь, атаки/защиты, отбой.
 * Только UI — логику/сокеты не трогаем.
 */
export default function Board({ trump, deckCount = 24, tableCards = [], discard = [] }) {
  const maxStack = Math.min(Number(deckCount) || 0, 6);
        ...
        <div className="relative w-full h-full">
          {Array.from({ length: maxStack }).map((_, i) => (
            <div
              key={i}
              className="deck-card"
              style={{ transform: `translate(${i * 3}px, -${i * 3}px) rotate(${i * 1.5}deg)` }}
            />
          ))}
        </div>
        {trump ? (
        ...
        <div className="relative w-[88%] h-[70%]">
          {tableCards.map((pair, idx) => (
            <motion.div
              ...
              className="absolute"
              style={{ left: `${10 + idx * 10}%`, top: `${18 + (idx % 2) * 22}%` }}
            >
              <div className="w-[90px] h-[130px] rounded-xl bg-neutral-700/60 border border-white/10 shadow-glow" />
              {pair?.defense && (
                <div className="absolute left-5 top-5 rotate-12 w-[90px] h-[130px] rounded-xl bg-neutral-800/80 border border-white/10" />
              )}
            </motion.div>
          ))}
          ...
          {!tableCards?.length && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-sm text-white/60">Ожидаем карты на столе…</div>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="absolute right-2 md:right-4 top-2 md:top-4 w-[140px] h-[120px]">
          <div className="relative discard-stack w-full h-full">
            {discard.slice(-6).map((c, i) => (
              <div
                key={i}
                className="card w-[90px] h-[130px] rounded-xl bg-neutral-700/60 border border-white/10"
                style={{ left: `${i * 12}px`, top: `${i * 6}px`, transform: `rotate(${6 + i * 4}deg)` }}
              />
            ))}
          </div>
        </div>
