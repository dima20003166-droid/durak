...
export default function RoomChat({ chat = [], myPlayer, onSend, openProfile }) {
  ...
  return (
    <div className="bg-surface rounded-2xl border border-border/60 p-4 chatPanel h-[520px]">
        ...
        {chat.map((m, i) => {
          ...
          return (
            <div key={i} className={`flex items-start gap-2 ${isMine ? 'justify-end' : ''}`}>
              ...
              <div className={`max-w-[80%] rounded-xl border border-border/60 px-3 py-2 text-sm ${isMine ? 'bg-primary/10' : 'bg-[#121230]'}`}>
      ...
      <div className="chatControls">
        <textarea
          className="flex-1 rounded-lg bg-[#0c0c22] border border-border/60 px-3 py-2 text-sm resize-none h-10 focus:outline-none focus:ring-2 focus:ring-primary/50"
          value={text}
