...
export default function CreateRoomModal({ onClose }) {
  ...
  const createRoom = () => {
    ...
    return (
      <div className="p-4 bg-surface rounded-xl shadow-lg text-text">
      ...
      <div className="flex gap-2">
          <button disabled={submitting} onClick={createRoom} className="px-4 py-2 bg-primary hover:bg-primary/80 rounded-md text-text font-semibold disabled:opacity-50">
            {submitting ? 'Создаю…' : 'Создать'}
          </button>
          <button onClick={onClose} className="px-4 py-2 bg-surface hover:bg-surface/80 rounded-md text-text">Отмена</button>
        </div>
      </div>
    );
  }
