...
const ModerationMenu = ({ menuData, onAction, onClose }) => {
    ...
    return (
        <div ref={menuRef} style={style} className="w-48 bg-gray-900 border border-gray-700 rounded-lg shadow-lg z-50">
            <button onClick={() => handleAction('delete')} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-800 rounded-t-lg">Удалить сообщение</button>
            <button onClick={() => handleAction('mute', 5)} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-800">Мут на 5 мин</button>
            <button onClick={() => handleAction('mute', 60)} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-800">Мут на 1 час</button>
            <button onClick={() => handleAction('deleteAll')} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-800 text-red-400 rounded-b-lg">Удалить все сообщения</button>
        </div>
    );
};
      ...
      <div className="bg-gray-800 text-white rounded-xl p-6 w-full max-w-sm">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          ...
          <div>
            ...
            <div className="text-xs text-gray-400">Поражения</div>
          </div>
          <div>
        ...
        <button
          onClick={onClose}
          className="mt-6 w-full bg-gray-700 hover:bg-gray-600 rounded-lg py-2 font-semibold"
        >
          Закрыть
        </button>
      </div>
...
const LobbyScreen = ({ user, onLogout, setPage, rooms, siteSettings }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col p-4 lg:p-8 text-gray-200">
      <header className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-emerald-400">DURAK.IO</h1>
      ...
      <main className="flex-grow flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-2/3 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => !iAmInAnyRoom && setShowCreateModal(true)} className="px-6 py-3 font-bold rounded-lg bg-emerald-600 hover:bg-emerald-700" disabled={iAmInAnyRoom} title={iAmInAnyRoom ? "Нельзя: у вас есть активная игра" : ""}>Создать игру</button>
          </div>

          <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl p-6 space-y-4 flex-grow">
            {rooms.map((room) => {
              const iAmHere = room.players.some((p) => p.socketId === mySocketId);
              return (
                <div key={room.id} className={`p-4 rounded-lg flex items-center justify-between border transition-all ${iAmHere ? "bg-gray-800/80 border-emerald-500 shadow-[0_0_0_3px_rgba(16,185,129,0.2)]" : "bg-gray-800 border-gray-700"}`}>
                  <div>
                  ...
                  <div className="flex items-center space-x-6">
                    ...
                    {iAmHere ? (
                      <>
                        <button onClick={() => setPage('game')} className="font-bold py-2 px-6 rounded-lg bg-blue-600 hover:bg-blue-700">
                          Вернуться
                        </button>
                        {(room.status === 'waiting' && room.creatorId === user.id) && (
                          <button
                            onClick={() => setCancelPrompt(room)}
                            className="font-bold py-2 px-4 rounded-lg bg-red-600 hover:bg-red-700"
                      ...
                      <button
                        ...
                        className="font-bold py-2 px-6 rounded-lg bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-600"
                      >
            ...
            {rooms.length === 0 && <p className="text-center text-gray-400">Открытых столов нет.</p>}
          </div>
        </div>

        {/* общий чат */}
        <div className="w-full lg:w-1/3 flex flex-col">
          <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl p-4 flex flex-col flex-grow">
            ...
            {chatMessages.map((msg, index) => { 
                ...
                const nameColor = userRole === 'admin' ? 'text-pink-400' : userRole === 'moderator' ? 'text-cyan-400' : 'text-white';
                ...
                return (
                    ...
                    <div className={`group flex items-center gap-2 ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
                        ...
                        {canModerate && 
                            <button onClick={(e) => handleOpenModerationMenu(e, msg)} className="self-center text-gray-500 hover:text-white opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <div className="flex items-center">
              <input
                ...
                className="flex-grow bg-gray-700 border border-gray-600 rounded-l-lg p-2"
              />
              <button
                onClick={handleSendMessage}
                className="bg-emerald-600 text-white font-bold p-2 rounded-r-lg hover:bg-emerald-700"
              >
                Отправить
              </button>
            </div>
          ...
          <div className="w-full max-w-md bg-gray-800 rounded-2xl p-8 text-center">
            ...
            <div className="flex justify-center gap-4 mb-6">
              <button
                ...
                className={`px-6 py-2 rounded-lg ${gameMode === 'Подкидной' ? 'bg-emerald-600' : 'bg-gray-700'}`}
              >
                Подкидной
              </button>
              <button
                onClick={() => setGameMode('Переводной')}
                className={`px-6 py-2 rounded-lg ${gameMode === 'Переводной' ? 'bg-emerald-600' : 'bg-gray-700'}`}
              >
                Переводной
              </button>
            </div>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                className="ml-2 bg-gray-700 rounded-lg px-4 py-2 w-32 text-right"
              />
            </div>

            <div className="flex gap-4">
              <button onClick={() => setShowCreateModal(false)} className="w-full py-3 bg-gray-600 rounded-lg">
                Отмена
              </button>
              <button onClick={handleCreateRoom} className="w-full py-3 bg-emerald-600 rounded-lg">
                Создать
              </button>
            </div>
          </div>
