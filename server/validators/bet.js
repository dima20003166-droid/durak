...
function validateBet(rawBet, userBalance) {
  ...
  if (!Number.isFinite(bet) || bet <= 0) {
    return { ok: false, code: 'ERR_BET_NOT_NUMBER', msg: 'Ставка должна быть числом больше 0.' };
  }
  ...
  if (bet > (userBalance ?? 0)) {
    return { ok: false, code: 'ERR_BET_EXCEEDS_BALANCE', msg: 'Ставка превышает ваш баланс.' };
  }
  return { ok: true, bet };
}
